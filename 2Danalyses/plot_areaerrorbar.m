...
% ----------------------------------------------------------------------- %
function hs = plot_areaerrorbar(data, options)

    % Default options
    if(nargin<2)
        options.fHandle     = figure(1);
    ...
    % Plotting the result
    figure(options.fHandle);
    x_vector = [options.x_axis', fliplr(options.x_axis')];
    hs(1) = fill(options.axHandle,x_vector, [data_mean+error,fliplr(data_mean-error)], options.color_area);
    set(hs(1), 'edgecolor', 'none');
    set(hs(1), 'FaceAlpha', options.alpha);
    hold on;
    hs(2) = plot(options.axHandle,options.x_axis, data_mean, 'color', options.color_line, ...
